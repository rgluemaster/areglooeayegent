...
public class AgentSmith implements AgentInterface {
    ...
    private static final boolean USE_TWO_ACTIONS = false;
...
//        System.out.println("Environment responded to \"what is your name?\" with: " + responseMessage);
        ...
        if(discountFactor == 1){
            discountFactor = 0.9999;//avoid overflow and strange behavior
        }
        ...
        if(nStates == 1) {
            ...
            if(Math.random()>1) {
                ...
                if(getAlpha(obs2.getInt(0)) < AGGREGATION_COUNT_THRESHOLD){
                    if(lastLastLastAction != null && USE_TWO_ACTIONS){
...
//    			newAction = randGenerator.nextInt(9);
        ...
        if(lastLastAction != null && USE_TWO_ACTIONS){
...
//        }
        ...
        System.out.println("Total number of episodes: " + episode);
        episode = 0;
        if(nStates==1){
    ...
    private int nextGSVIAction(Observation observation) {
        ...
        if(n>0) {
            //e = 1.0/10.0;
            double exponent = nStates == 2188 ? -0.5 : -2;
            e = Math.pow(n, -5);
            //e = 1.0/n;
        }
        int returnAction = 0;
        if(randGenerator.nextDouble() < e) {
            ...
            returnAction = GSVI_pi[thisState];
        }

        System.out.print("Actions:");
        for(int j = 0; j < nActions; j++){
            System.out.print(Util.roundNDecimals(GSVI_Q[thisState][j],4) + ", ");
        }
        System.out.println("Chosen action: " + Util.roundNDecimals(GSVI_Q[thisState][returnAction], 4));

        System.out.print("Mean:");
        for(int j = 0; j < nActions; j++){
            System.out.print(Util.roundNDecimals(rewards[thisState][j].getMean(),8) + ", ");
        }
        System.out.println("Chosen mean: " + Util.roundNDecimals(rewards[thisState][returnAction].getMean(), 8));
    ...
    private void generalizedStochasticValueIteration() {
        ...
        while(visibleStatesIterator.hasNext()){
            ...
            for(int a = 0; a<nActions;a++) {
                ...
                if(rewards[s][a].getSampleSize() > 0){
                    bonus = rewards[s][a].getSample().get(rewards[s][a].getSampleSize() - 1);
                }


                actionValue[a] = Qtemp[s][a]*(1-learningRate) + learningRate*(rewards[s][a].getMean()+bonus);
                Iterator<Integer> nextStatesIterator = visibleStates.iterator();
                while(nextStatesIterator.hasNext()){
                    int ss = nextStatesIterator.next();
                    double p = 0;
